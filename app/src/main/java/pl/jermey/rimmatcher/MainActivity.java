...
//        Stream.of(rimInfoList).forEach(rimInfo -> viewPagerAdapter.add(rimInfo));
        ...
        pagerContainer.getViewPager().setAdapter(viewPagerAdapter);
        pagerContainer.getViewPager().setOffscreenPageLimit(5);
        pagerContainer.getViewPager().setClipChildren(false);
        pagerContainer.getViewPager().setClipToPadding(false);
        pagerContainer.getViewPager().setPageMargin((int) -(getResources().getDisplayMetrics().widthPixels * 0.2f));
        pagerContainer.getViewPager().setPageTransformer(true, (page, position) -> {
